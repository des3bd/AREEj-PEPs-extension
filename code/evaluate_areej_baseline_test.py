...
INPUT_CSV = Path("data/final/pep_areej_baseline_test_predictions.csv")
OUTPUT_CSV = Path("data/final/pep_areej_baseline_test_evaluated.csv")
SUMMARY_TXT = Path("data/final/pep_areej_baseline_test_summary.txt")
...
def relation_match(pred_relation: str, gold_relation: str) -> bool:
    ...
    return pred in acceptable and gold in acceptable


def parse_first_areej_relation(prediction: str) -> dict:
    """
    Parses the first AREEj-style relation.
    ...
    if not match:
        # Fallback if the model produced malformed output
...
def main():
    ...
    parsed_rows = [parse_first_areej_relation(pred) for pred in df[PRED_COL]]
    ...
    lines = []
    lines.append("==============================")
    lines.append("AREEj BASELINE TEST EVALUATION")
    lines.append("==============================\n")
