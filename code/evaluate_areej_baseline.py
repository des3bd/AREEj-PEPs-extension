...
INPUT_CSV = Path("data/final/pep_areej_baseline_predictions.csv")
OUTPUT_CSV = Path("data/final/pep_areej_baseline_evaluated.csv")
SUMMARY_TXT = Path("data/final/pep_areej_baseline_evaluation_summary.txt")

PRED_COL = "areej_baseline_prediction"
...
def contains_match(long_text: str, short_text: str) -> bool:
    """
    Partial containment match after Arabic normalization.
    Used because AREEj may extract partial names or normalized text.
    """
...
def token_overlap_score(a: str, b: str) -> float:
    ...
    Useful for partial evidence/object matching.
    """
    a_tokens = set(normalize_arabic(a).split())
    b_tokens = set(normalize_arabic(b).split())
...
def parse_first_areej_relation(prediction: str) -> dict:
    """
    Parses the first relation from AREEj-style output.

    Expected style:
    <bor> subject <per> object <concept> relation <rt> evidence <e>

    AREEj may output different entity type tags:
    <per>, <org>, <loc>, <concept>, <eve>, <time>, etc.
    """
    ...
    # Entity type tags that may appear in AREEj
    type_tags = [
        "per", "org", "loc", "concept", "eve", "time",
        "misc", "product", "work", "law", "language"
    ]

    type_pattern = "|".join(type_tags)

    # Pattern:
    # subject <type1> object <type2> relation <rt> evidence
    pattern = rf"^(.*?)<({type_pattern})>(.*?)<({type_pattern})>(.*?)<rt>(.*)$"

    match = re.search(pattern, relation_block)

    if not match:
        # fallback: try to at least get relation/evidence
...
def relation_match(pred_relation: str, gold_relation: str) -> bool:
    ...
    # Your gold relation is position_held, while AREEj often uses "position held"
    acceptable = {
        "position held",
        "position_held",
        "held position",
...
def main():
    ...
    for _, row in df.iterrows():
        parsed = parse_first_areej_relation(row[PRED_COL])
    ...
    df["evidence_overlap"] = df.apply(
        lambda row: token_overlap_score(row["pred_evidence"], row["evidence"]),
        axis=1
    )

    # Evidence partial match threshold
    df["evidence_partial_match"] = df["evidence_overlap"] >= 0.50

    df["full_triple_match"] = (
        df["subject_match"]
        & df["object_match"]
        & df["relation_match"]
    )

    df["full_relation_with_evidence_match"] = (
        df["subject_match"]
        & df["object_match"]
        & df["relation_match"]
        & df["evidence_partial_match"]
    )
    ...
    metrics = {
        ...
        "evidence_partial_match_overlap_0_50": df["evidence_partial_match"].mean(),
        "full_triple_match": df["full_triple_match"].mean(),
        "full_relation_with_evidence_match": df["full_relation_with_evidence_match"].mean(),
    }

    lines = []
    lines.append("==============================")
    lines.append("AREEj BASELINE EVALUATION")
    lines.append("==============================\n")
    ...
    wrong = df[~df["full_triple_match"]].head(10)
    ...
    if wrong.empty:
        ...
        for _, row in wrong.iterrows():
            ...
            lines.append(
                ...
                f"evidence_partial={row['evidence_partial_match']}"
            )
